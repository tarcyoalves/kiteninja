...
Data: 2026-08-23 · Correções aplicadas sobre `ddadbf1` (`main`, branch de produção)
Produção: https://kiteninja.vercel.app

> **Atenção ao branch.** A auditoria começou em `master`, que se revelou **defasado em
> 50+ commits** (290 arquivos, ~37 mil linhas de diferença: rede social, downwind ao
> vivo, DM, perfis públicos). **A produção sai de `main`.** Corrigir `master` não muda o
> app no ar — todos os achados foram reverificados e reaplicados em `main`. Ver D4.

Cada achado é **CONFIRMADO** (prova de execução ou evidência direta), **PROVÁVEL** ou
**POSSÍVEL**. Nada aqui é hipótese apresentada como bug.

**Escopo da varredura em profundidade:** 39 rotas, 25 tabelas e 4.018 linhas de `lib/`,
como estavam em `master`. **Escopo real de `main`:** 58 rotas, 33 tabelas, 12.538 linhas
de `lib/`, 36 arquivos de teste. Os achados abaixo foram todos reverificados contra `main`
e valem para o código em produção, mas os módulos exclusivos de `main` — downwind ao vivo,
DM, grafo social, feed com keyset — **não passaram pela varredura em profundidade** e
merecem uma auditoria própria. Registro isso explicitamente para não passar por cobertura
que não existe.
...
```
app/api/        58 rotas (39 na base varrida em profundidade)
lib/            auth · authz · rateLimit · weather · sos · sosCandidates · push · chat
components/ views/ context/
scripts/        migrate · seed · verify-sql (PGlite) · verify-db (Neon)
```
...
| C8 | Teste de inbox de DM com corrida de timestamp (flaky) | **corrigido nesta auditoria** — ver abaixo |

**C8 · Teste flaky no `verify-sql` — CONFIRMADO por observação direta.** As duas mensagens
de DM eram inseridas com `DEFAULT NOW()` no mesmo instante, empatando `created_at`. O teste
seguinte afirma que o inbox mostra "a mensagem mais recente", mas com empate o
`ORDER BY created_at DESC` não tem desempate e o resultado dependia da ordem física das
linhas. **Foi visto falhando de verdade** durante esta auditoria (218/1) e passando nas
rodadas seguintes. Um teste que falha sem regressão é pior que teste ausente: ensina a
ignorar o vermelho. Corrigido com `created_at` explícito e 1 minuto de distância entre as
mensagens; verificado estável em 3 rodadas.
...
| D4 | `master` defasado 50+ commits de `main`, que é a branch de produção | **ABERTO** — risco alto, ver §4 |
...
**D4 · Dois branches divergentes (risco de processo, alto).** `master` está 50+ commits
atrás de `main`, com 290 arquivos e ~37 mil linhas de diferença. **A produção sai de
`main`.** Isto já custou trabalho nesta própria auditoria: as primeiras correções foram
para `master` e não teriam chegado ao app no ar. Pior, um agente que leia `master` como
"o código" audita um app que não existe mais. *Proposta:* aposentar `master` — ou apagá-lo
no remoto, ou apontá-lo para `main` — e deixar uma única linha de verdade. Não executo
sem seu aval, porque mexer em branch remoto é irreversível.
